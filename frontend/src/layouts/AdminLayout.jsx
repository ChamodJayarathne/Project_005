  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        // Then validate with the server and get updated data
        const response = await axios.get(
          `${baseUrl}/api/protected/current-user`,
          {
            headers: { Authorization: `Bearer ${token}` },
        ...
        // Normalize user data structure - IMPORTANT: Include profileImage
        const normalizedUser = {
          id: response.data?.id || response.data?._id,
          username: response.data?.username,
          role: response.data?.role,
          email: response.data?.email,
          profileImage: response.data?.profileImage, // Make sure this is included
            ...
            <div className="relative">
              {userData?.profileImage ? (
                <>
                  ...
                  <div
                    className="bg-gray-200 border-2 border-dashed rounded-full w-16 h-16 flex items-center justify-center absolute top-0 left-0"
                    style={{ display: 'none' }}
                  >
        ...
        <nav className="flex-1 px-6 mt-6">
          <ul className="space-y-6">
            <li>
              <Link
                to="/admin"
                className={`text-white py-3 px-6 rounded-full w-full text-left flex items-center ${location.pathname === "/admin" ? "bg-gray-600" : ""
                  }`}
              >
                <FiHome className="mr-3" />
                <span>Dashboard</span>
            ...
            <li>
              <Link
                to="/admin/add-post"
                className={`text-white py-3 px-6 rounded-full w-full text-left flex items-center ${location.pathname === "/admin/add-post"
                  ? "bg-gray-600"
                  : ""
                  }`}
              >
            ...
            <li>
              <Link
                to="/admin/orders"
                className={`text-white py-3 px-6 rounded-full w-full text-left flex items-center ${location.pathname === "/admin/orders"
                  ? "bg-gray-600"
                  : " "
                  }`}
              >
            ...
            <li>
              <Link
                to="/admin/summary"
                className={`text-white py-3 px-6 rounded-full w-full text-left flex items-center ${location.pathname === "/admin/summary"
                  ? "bg-gray-600"
                  : ""
                  }`}
              >
            ...
            <li>
              <Link
                to="/admin/accounts"
                className={`text-white py-3 px-6 rounded-full w-full text-left flex items-center ${location.pathname === "/admin/accounts"
                  ? "bg-gray-600"
                  : " "
                  }`}
              >
